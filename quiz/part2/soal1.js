...
function digitPerkalianMinimum(angka) 
{
  // you can only write your code here!
    ...
    for (let i = 1; i <= angka; i++)
    {
        if (angka % i === 0)
        {
            let stringAngka1 = i.toString().split('');
            let stringAngka2 = (angka / i).toString().split('');

            if (jumlahDigit > (stringAngka1.length + stringAngka2.length) || jumlahDigit === -1)
            {
                jumlahDigit = (stringAngka1.length + stringAngka2.length);
            }
        }
    }
