...
function countProfit(shoppers) 
{
  ...
  // you can only write your code here!
    ...
    for (let i = 0; i < listBarang.length; i++)
    {
        ...
        listShopping.push(tempBarangShoppers);
    }
    ...
    {
        for (let j = 0; j < listShopping.length; j++)
        {
            if (shoppers[i].product === listShopping[j].product)
            {
                if (listShopping[j].leftOver >= shoppers[i].amount)
                {
                    let tempLeftOver = listShopping[j].leftOver - shoppers[i].amount;
                    listShopping[j].leftOver = tempLeftOver;
                    
                    listShopping[j].shoppers.push(shoppers[i].name);

                    let tempTotalProfit = listShopping[j].totalProfit;
                    tempTotalProfit += (shoppers[i].amount * listShopping[j].harga); 
                    listShopping[j].totalProfit = tempTotalProfit;
                }
            }
